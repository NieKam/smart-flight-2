...
class NearbyCityControllerTest {
    ...
    @Test fun `newer result wins while an older lookup is active`() {
        ...
        controller.start(); controller.onLocationFix(FlightLocationFix(null, null, 1, latitude = 1.0, longitude = 1.0))
    ...
    @Test fun `repository failures invalid zones and empty results are unavailable`() {
        val states = mutableListOf<NearbyCityState>()
        val invalidZone = NearbyCityController(FakeRepository(listOf(city(1, "Bad", 0.0, 0.0).copy(timeZoneId = "Not/AZone"))), direct, direct, { Instant.EPOCH }, states::add)
        invalidZone.start(); invalidZone.onLocationFix(FlightLocationFix(null, null, 1, latitude = 0.0, longitude = 0.0))
        assertEquals(NearbyCityState.Unavailable, states.last())

        val failed = NearbyCityController(object : NearbyCityRepository {
            override fun findNearest(position: NearbyCoordinate, reload: Boolean): NearbyCityRecord? = throw IllegalStateException()
        }, direct, direct, { Instant.EPOCH }, states::add)
        failed.start(); failed.onLocationFix(FlightLocationFix(null, null, 1, latitude = 0.0, longitude = 0.0))
        assertEquals(NearbyCityState.Unavailable, states.last())

        val empty = NearbyCityController(FakeRepository(emptyList()), direct, direct, { Instant.EPOCH }, states::add)
        empty.start(); empty.onLocationFix(FlightLocationFix(null, null, 1, latitude = 0.0, longitude = 0.0))
        assertEquals(NearbyCityState.Unavailable, states.last())
    }
    ...
    @Test fun `retry reloads the latest position and successful time keeps offset data`() {
        ...
        controller.start(); controller.onLocationFix(FlightLocationFix(null, null, 1, latitude = 0.0, longitude = 0.0))
        assertEquals(NearbyCityState.Unavailable, states.last())
        controller.retry()
