...
fun convertAltitude(
    value: Double,
    unit: AltitudeUnit,
): Double? = value.convert { if (unit == AltitudeUnit.FEET) it * 3.28084 else it }

fun convertDistance(
    value: Double,
    unit: DistanceUnit,
): Double? = value.convert { if (unit == DistanceUnit.MILES) it * 0.621371 else it }

fun convertVerticalSpeed(
    value: Double,
