...
- Parity "Not verified": how many users lack a rotation-vector sensor is unknown; the fallback is cheap, so it is planned regardless. Human decision: this task keeps its position in the parity phase (after the architecture migration, same relative order).
